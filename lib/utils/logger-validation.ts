...
/**
 * Valida la correcta implementación de logging en un endpoint
 */
export function validateEndpointLogging(
  endpoint: string,
  expectedLogTypes: ('info' | 'warn' | 'error' | 'debug')[] = ['info', 'error']
): ValidationResult {
  const result: ValidationResult = {
    isValid: true,
    severity: 'low',
  ...
  // Esta validación sería más útil en un entorno de pruebas
  // Aquí solo podemos emitir advertencias
  if (!expectedLogTypes.includes('error')) {
    result.warnings.push(`Se recomienda incluir 'error' en los tipos de log esperados para endpoint ${endpoint}`)
  }

  if (!expectedLogTypes.includes('info')) {
    result.warnings.push(`Se recomienda incluir 'info' en los tipos de log esperados para endpoint ${endpoint}`)
  }

  // Aplicar análisis predictivo según tipo de endpoint
  if (endpoint.includes('auth') || endpoint.includes('login') || endpoint.includes('register')) {
    if (!expectedLogTypes.includes('info')) {
      result.suggestions.push(`Endpoint de autenticación debería incluir logs de tipo 'info' para auditoría`)
      result.severity = 'medium'
    }
  }

  if (endpoint.includes('payment') || endpoint.includes('billing') || endpoint.includes('checkout')) {
    if (!expectedLogTypes.includes('info') || !expectedLogTypes.includes('error')) {
      result.suggestions.push(`Endpoint de pago debería incluir logs de tipo 'info' y 'error' para trazabilidad`)
      result.severity = 'high'
    }
  }
